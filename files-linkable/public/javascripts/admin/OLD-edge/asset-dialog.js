...
AssetDialog.EdgeDelegate.BASE_URL = '/admin/assets/';
...
AssetDialog.EdgeDelegate.EXTENSION_MAP = {
    
	// executables
	com: 'application_xp_terminal', exe: 'application', bat: 'application',

	// code
    h: 'page_white_h', c: 'page_white_c', cpp: 'page_white_cplusplus',
    cs: 'page_white_csharp', php: 'page_white_php', rb: 'page_white_ruby',
    rhtml: 'page_white_code_red', cfm: 'page_white_coldfusion', as: 'page_white_actionscript',
    html: 'page_white_world', xml: 'page_white_code', java: 'page_white_cup',
    sql: 'page_white_database',
    
    // editable images
    jpg: 'page_white_picture', jpeg: 'page_white_picture', gif: 'page_white_picture',
    png: 'page_white_picture',
    
    // other images
    bmp: 'page_white_paint', psd: 'page_white_paint', tif: 'page_white_paint',
    tiff: 'page_white_paint', ai: 'page_white_vector',
    
    // documents
    ppt: 'page_white_powerpoint', pdf: 'page_white_acrobat', xls: 'page_white_excel',
    doc: 'page_white_word', mdb: 'page_white_key', fla: 'page_white_flash',
    swf: 'page_white_flash', txt: 'page_white_text',
    
    // archives
    zip: 'page_white_compressed', gz: 'page_white_compressed',
	bz2: 'page_white_compressed', tar: 'page_white_zip', iso: 'page_white_cd',
    
    // urls
    url: 'page_white_world', webloc: 'page_white_world',
    
    // audio
    wav: 'sound', aiff: 'sound', aif: 'sound', mp3: 'sound', ogg: 'sound',
    
    // video
    wmv: 'film', mpg: 'film', mpeg: 'film', avi: 'film', mov: 'film', flv: 'film'
...
AssetDialog.EdgeDelegate.prototype = {
	...
	jsonToEntry: function(data) {
		var obj = data.asset_folder || data.asset;
		    obj.type = data.asset_folder ? 'folder' : 'asset';
	...
	getUrlForEntry: function(entry) {
		return entry.folder ? '' : ('/assets/' + entry.id);
	},
	
	getTypeIconForEntry: function(entry) {
		var i = "/images/icons/silk/";
		if (entry.folder) {
			if(entry.id) {
	...
	getIconForEntry: function(entry) {
		return "/assets/show/" + entry.id + "/system-icon";
	},
	
	getRawThumbnailForEntry: function(entry) {
		if(entry.asset && entry.webSafe) {
			return "/assets/show/" + entry.id + "/system-thumbnail";
		} else {
			return null;
		}
	},
	...
	getThumbnailForEntry: function(entry) {
		if(entry.asset && (entry.webSafe || entry.thumbnail_id)) {
			return "/assets/thumb/" + entry.id + "/system-thumbnail";
		} else {
			return null;
		}
	},
	...
	getSWFUploadURL: function() {
		return '/images/admin/swfupload.swf';
	},
	...
	getSWFUploadButtonURL: function() {
		return '/images/admin/upload-button.png';
	},
	...
	getUploadPostName: function() {
		return this.getAttributePostName('uploaded_data');
	},
